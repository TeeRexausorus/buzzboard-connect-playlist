...
export const BuzzerCard = ({ id, name, state, pressedAt }: BuzzerCardProps) => {
  ...
  return (
    ...
    >
      <Card className={`p-6 relative overflow-hidden transition-all duration-300 ${colors[state]}`}>
        {state === "pressed" && (
          <motion.div
        ...
        <div className="relative z-10">
          <div className="flex justify-between items-start mb-4">
            <h3 className="text-3xl font-display font-bold text-foreground">{name}</h3>
          </div>
          ...
          {state === "waiting" && <div className="flex items-center gap-2 text-muted-foreground"></div>}
